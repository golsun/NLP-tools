...
for(my $n=1;$n<=4;$n++) {
  if (defined ($TOTAL[$n])){
    $prec[$n]=($TOTAL[$n])?$CORRECT[$n]/$TOTAL[$n]:0;
...
$bleu[4] = $brevity_penalty * exp((my_log( $prec[1] ) + my_log( $prec[2] ) + my_log( $prec[3] ) + my_log( $prec[4])) / 4);
$bleu[3] = $brevity_penalty * exp((my_log( $prec[1] ) + my_log( $prec[2] ) + my_log( $prec[3])) / 3);
$bleu[2] = $brevity_penalty * exp((my_log( $prec[1] ) + my_log( $prec[2])) / 2);
$bleu[1] = $brevity_penalty * exp((my_log( $prec[1])));
printf "BLEU = %.2f, bleu=%.2f/%.2f/%.2f/%.2f prec=%.2f/%.2f/%.2f/%.2f (BP=%.3f, ratio=%.3f, hyp_len=%d, ref_len=%d)\n",
    100*$bleu[4],
    100*$bleu[1],
    100*$bleu[2],
    100*$bleu[3],
